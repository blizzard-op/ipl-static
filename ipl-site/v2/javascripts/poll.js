...
(function() {
  var Poll, authCheck, checkForNewPolls, fetchNewPoll, getPoll, getPolls, getStreams, loadPolls, loadUser, newPollInterval, restrictedCountries, returningVote, updatingVote, user, _mediaUrl, _url;
  ...
  Poll = (function() {
    ...
    Poll.prototype.getPayouts = function(userId) {
      ...
      fetchingPayouts = $.ajax({
        url: "http://" + _url + "/vote/v2/polls/" + this.poll.id + "/stats/users/" + user.id,
      ...
      fetchingPayouts.done(function(data) {
        if (user.id === 21) {
          ...
          _this.votedValue = data.votedFor;
          return _this.$el.find("i").each(function(index) {
            var $this;
            $this = $(this);
            if ($this.data("value") === data.votedFor) {
    ...
    Poll.prototype.postVote = function() {
      var postingVote,
        _this = this;
      postingVote = $.ajax({
        url: "http://" + _url + "/vote/v2/votes/" + this.poll.id,
        type: "POST",
        dataType: "json",
        data: {
          option: {
      ...
      postingVote.done(function(data) {
        ...
        for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
          player = _ref[index];
          if (player.selected === true && user.id !== 21) {
  ...
  loadUser = function(poll, userData) {
    ...
    if (userData === null) {
      ...
      return poll.getPayouts();
    }
  };

  loadPolls = function(pollData, streamId) {
    var checkingAuth, poll, _i, _len;
    if (!pollData.length) {
      return;
    }
